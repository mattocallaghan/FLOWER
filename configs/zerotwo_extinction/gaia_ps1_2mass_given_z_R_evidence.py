...
with distance prior and importance sampling for Gaia + PS1 + 2MASS data.
"""

def get_config():
    # Start with the base configuration.
    config = base_get_config()
    config.training.workspace='output/02_extinction_g_ps1_2mass_evidence'
    ...
    ### model parameters for gaia_ps1_2mass data
    config.model.name = 'nlpe_bnaf_elu'
    config.model.nn_depth_bnaf = 3
    config.model.activation='tanh'
    config.model.flow_dimension=11  # M_G + 10 colors: M_G, BP-G, RP-G, g_PS1-G_Gaia, r_PS1-G_Gaia, i_PS1-G_Gaia, z_PS1-G_Gaia, y_PS1-G_Gaia, J-G_Gaia, H-G_Gaia, Ks-G_Gaia
    config.model.nn_block_dim=4
    config.model.cond_dim=2  # log(|Z|), log(R)
    config.data.vector_dim=11  # M_G + colors (in physical units)
    config.data.vector_dim_inference=11
    config.data.summary_dim=0  # no summary statistics needed

    config.data.dataset='gaia_ps1_2mass'
    config.data.twomass=True  # Flag to indicate 2MASS data is included
    config.data.benchmark = False  # We're not using SBIBM data
    config.data.save_csv=False
    config.data.save_path_gaia_data = "Data/gaia_ps1_2mass_data_018_evidence.csv"  #if saving then the location
    config.data.inference_dataset='inference_val_gaia_ps1_2mass_018'

    # Updated data paths for 2MASS data
    #config.data.data_path = "/Users/mattocallaghan/FLOWER_FLOW/Data/gaia_ps1_2mass.npy"
    #config.data.stats_path = "/Users/mattocallaghan/FLOWER_FLOW/Data/gaia_ps1_2mass_stats.npz"
    ...
    config.loss.use_contrastive = True        # Enable contrastive learning for zero-extinction star detection
    ...
    config.sampling.max_tree_depth=10
    config.sampling.bands=['G','BP','RP','g','r','i','z','y','J','H','Ks']  ## what bands to actually do the inference over including 2MASS
