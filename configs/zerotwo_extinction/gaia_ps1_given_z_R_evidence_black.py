...
def get_config():
    ...
    config.training.workspace='output/02_extinction_g_ps1_evidence_black'
    ...
    config.data.inference_dataset='inference_val_gaia_ps1_black'
    ...
    config.loss.contrastive_weight = 0.7    # Weight for contrastive loss term  
    config.loss.contrastive_samples = 5       # Number of reddened samples per clean star
    config.loss.contrastive_clamp_bound = -70.0  # Clamp bound to prevent CNF mode collapse (literature recommendation)

    #### training
    config.training.batch_size =256*2*2*2*2 # Moderate batch size for stable training
